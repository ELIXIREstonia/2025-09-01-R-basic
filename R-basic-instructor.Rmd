...
You can find this and the data files from ELIXIREstonia github. <https://github.com/ELIXIREstonia/2024-10-02-R-basic>.
...
bio_names <- c("Anna", "Mike", "Luis")
bio_ages <- c(20, 21, 19)
bio_scores <- c(85, 78, 90)
print("Biology vectors:")
print(bio_names)
print(bio_ages)
print(bio_scores)
```
...
all_ages <- c(20, 21, 19, 22, 20, 23, 24, 22, 25)
all_scores <- c(85, 78, 90, 88, 76, 95, 87, 80, 92)
student_matrix <- matrix(c(all_ages, all_scores), ncol = 2, dimnames = list(NULL, c("Age", "Score")))
...
all_names <- c("Anna", "Mike", "Luis", "Sam", "Ella", "Ravi", "Mia", "Jack", "Olivia")
all_classes <- c("Biology", "Biology", "Biology", "Chemistry", "Chemistry", "Chemistry", "Physics", "Physics", "Physics")
...
high_scores <- student_df[student_df$score > 85, ]
print(high_scores[, c("name", "class")])
```

```{r}
# - Average score per class from the matrix
avg_score_bio <- mean(student_matrix[all_classes == "Biology", "Score"])
avg_score_chem <- mean(student_matrix[ all_classes == "Chemistry", "Score"])
avg_score_physics <- mean(student_matrix[all_classes == "Physics", "Score"])
print("Average scores per class (from matrix):")
print(paste("Biology:", avg_score_bio))
print(paste("Chemistry:", avg_score_chem))
print(paste("Physics:", avg_score_physics))
```
...
Isd %>% 
  summarise(
    avg_before = mean(Mem_Score_Before),
    avg_after = mean(Mem_Score_After)
  )
...
# 3. Dosage Analysis
print("Dosage analysis:")
Isd %>%
  group_by(Drug, Dosage) %>%
  summarise(
    avg_diff = mean(Diff)
  ) %>%
  arrange(Drug, desc(avg_diff))
...
# 5. Happy/Sad Group Analysis
print("Happy/Sad group analysis:")
Isd %>%
